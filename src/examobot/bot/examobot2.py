...
async def handle_back_to_test_question_from_task_query(call: types.CallbackQuery):
    ...
    if not test:
        await call.bot.edit_message_text("Тест был удален автором",
                                         call.from_user.id,
...
async def send_test_to_classroom_participants(test_id: int, classroom_id: int, bot: Bot) -> None:
    ...
    for participant in participants:
        ...
        await send_message_to_user(bot, participant.id, "Доступен новый тест")
...
async def handle_message(message: types.Message):
    ...
    if not test:
        await message.bot.edit_message_text("Тест был удален автором",
                                            message.from_user.id,
